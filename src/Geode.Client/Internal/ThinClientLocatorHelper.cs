...
{
    /// <summary>
    /// cppcache <c>ThinClientLocatorHelper.cpp:117</c> ??magic int prefix to every locator request.
    /// </summary>
    private const int GossipVersion = 1002;

    /// <summary>
    /// cppcache <c>TcrConnection.hpp:44</c>: first byte the locator sends when it requires SSL but the client did not enable TLS.
    /// </summary>
    ...
    /// <summary>
    /// Lock + copy + shuffle. Mirrors cppcache <c>getLocators()</c> (<c>ThinClientLocatorHelper.cpp:75-85</c>).
    /// </summary>
        ...
        CancellationToken ct) where T : class
    {
        try
        {
            ...
            while (true)
            {
                try
                {
                    var reader = new DataInput(buffer.AsMemory(0, totalRead));
    ...
    /// response.
    /// </summary>
    private static void ReadEnvelope(DataInput reader, DSFid expectedDsfid)
    {
