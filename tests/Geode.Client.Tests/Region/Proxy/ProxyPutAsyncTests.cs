...
/// </summary>
public class ProxyPutAsyncTests(IGeodeCacheFactory factory)
{
    ...
    {
        const string cacheName = nameof(ProxyPutAsyncTests);
        try
        {
    ...
    {
        const string cacheName = nameof(ProxyPutAsyncTests) + "_callback";
        try
        {
            var cache = await NewCacheAsync(cacheName);
            await cache.PoolManager.CreateFactory()
