...
public interface IRegion
{
    ...
    /// <summary>
    /// Live <see cref="IPool"/> this region dispatches through. Mirrors
    /// cppcache <c>Region::getPool()</c>
    /// (<c>cppcache/include/geode/Region.hpp:1449</c>). Shared with every
    /// other region created against the same <see cref="PoolName"/>;
    /// callers must not <see cref="IAsyncDisposable.DisposeAsync"/> it
    /// — pool lifetime is owned by <see cref="IGeodeCache"/>.
    /// </summary>
    IPool Pool { get; }

    /// <summary>Parent region; <see langword="null"/> at root.</summary>
    IRegion? ParentRegion { get; }

    /// <summary>Returns the sub-region at <paramref name="path"/>, or <see langword="null"/> if absent.</summary>
    IRegion? GetSubregion(string path);

    /// <summary>Creates a sub-region named <paramref name="name"/> with <paramref name="attributes"/>.</summary>
    IRegion CreateSubregion(string name, RegionAttributes attributes);

    /// <summary>Sub-regions immediately under this one (or transitively when <paramref name="recursive"/> is <see langword="true"/>).</summary>
    IReadOnlyList<IRegion> Subregions(bool recursive);

    /// <summary>Destroys this region locally only (server stays unchanged); <paramref name="callback"/> reaches local <c>CacheWriter</c>/<c>CacheListener</c>.</summary>
    void LocalDestroyRegion(object? callback = null);

    /// <summary>Local-only put against the in-memory entry map (no server roundtrip).</summary>
    void LocalPut(object key, object value, object? callback = null);

    /// <summary>Local-only strict insert; throws if <paramref name="key"/> already exists.</summary>
    void LocalCreate(object key, object value, object? callback = null);

    /// <summary>Local-only invalidate (value cleared, key stays).</summary>
    void LocalInvalidate(object key, object? callback = null);

    /// <summary>Local-only destroy of <paramref name="key"/>.</summary>
    void LocalDestroy(object key, object? callback = null);

    /// <summary>Local-only remove of <paramref name="key"/>/<paramref name="value"/>; <see langword="true"/> when removed.</summary>
    bool LocalRemove(object key, object value, object? callback = null);

    /// <summary>Local-only remove of <paramref name="key"/> ignoring value; <see langword="true"/> when removed.</summary>
    bool LocalRemoveEx(object key, object? callback = null);

    /// <summary>Clears every entry from the local entry map only.</summary>
    void LocalClear(object? callback = null);

    /// <summary>Invalidates every entry's value locally (keys stay).</summary>
    void LocalInvalidateRegion(object? callback = null);

    /// <summary>Keys this client has registered subscription interest in.</summary>
    IReadOnlyList<object> GetInterestList();

    /// <summary>Regular expressions this client has registered subscription interest in.</summary>
    ...
    /// <summary>Unregisters previously-registered subscription interest in <paramref name="keys"/>.</summary>
    Task UnregisterKeysAsync(IReadOnlyCollection<object> keys, CancellationToken ct = default);

    /// <summary>Registers subscription interest in every key on the server. See <see cref="RegisterKeysAsync"/> for the flag semantics.</summary>
    Task RegisterAllKeysAsync(
        bool isDurable = false,
        bool getInitialValues = false,
        bool receiveValues = true,
        CancellationToken ct = default);

    /// <summary>Unregisters the all-keys subscription.</summary>
    Task UnregisterAllKeysAsync(CancellationToken ct = default);

    /// <summary>Registers subscription interest in keys matching <paramref name="regex"/>. See <see cref="RegisterKeysAsync"/> for the flag semantics.</summary>
    ...
    /// <summary>Unregisters the previously-registered <paramref name="regex"/>.</summary>
    ...
    /// <summary>Conditional remove: removes <paramref name="key"/> only when its current value equals <paramref name="value"/>; mirrors cppcache <c>Region::remove(key, value, cb)</c>.</summary>
    Task<bool> RemoveAsync(object key, object value, object? callback = null, CancellationToken ct = default);

    /// <summary>Unconditional remove of <paramref name="key"/>; mirrors cppcache <c>Region::removeEx(key, cb)</c>. Returns <see langword="true"/> when the key existed.</summary>
    ...
    /// <summary>Strict insert; throws when <paramref name="key"/> already exists (cppcache <c>Region::create</c>).</summary>
    Task CreateAsync(object key, object value, object? callback = null, CancellationToken ct = default);

    /// <summary>Strict remove; throws when <paramref name="key"/> is absent (cppcache <c>Region::destroy</c>).</summary>
    Task DestroyAsync(object key, object? callback = null, CancellationToken ct = default);

    /// <summary>Destroys the whole region on the server (mirrors cppcache <c>Region::destroyRegion</c>).</summary>
    Task DestroyRegionAsync(object? callback = null, CancellationToken ct = default);

    /// <summary>Invalidates every entry in the region on the server (keys stay, values cleared).</summary>
    Task InvalidateRegionAsync(object? callback = null, CancellationToken ct = default);

    /// <summary>Local <see cref="IRegionEntry"/> snapshot for <paramref name="key"/>; <see langword="null"/> when absent locally.</summary>
    IRegionEntry? GetEntry(object key);

    /// <summary>Snapshot of locally-cached keys.</summary>
    IReadOnlyList<object> Keys();

    /// <summary>Snapshot of keys defined on the server for this region.</summary>
    Task<IReadOnlyList<object>> ServerKeysAsync(CancellationToken ct = default);

    /// <summary>Snapshot of locally-cached values (invalidated entries skipped).</summary>
    IReadOnlyList<object?> Values();

    /// <summary>Snapshot of <see cref="IRegionEntry"/> entries; descends into sub-regions when <paramref name="recursive"/> is <see langword="true"/>.</summary>
    IReadOnlyList<IRegionEntry> Entries(bool recursive);

    /// <summary>Number of entries currently in the local cache.</summary>
    int Size { get; }

    /// <summary>Whether this region has been destroyed.</summary>
    bool IsDestroyed { get; }

    /// <summary>Attributes snapshot taken at region creation time.</summary>
    RegionAttributes Attributes { get; }

    /// <summary>Mutator for the subset of attributes adjustable after creation.</summary>
    IAttributesMutator GetAttributesMutator();

    /// <summary>Local containment check — does the local cache hold <paramref name="key"/>?</summary>
    bool ContainsKey(object key);

    /// <summary>Local containment check for a non-invalidated value under <paramref name="key"/>.</summary>
    bool ContainsValueForKey(object key);

    /// <summary>Parent <see cref="IRegionService"/> (the cache) this region belongs to.</summary>
