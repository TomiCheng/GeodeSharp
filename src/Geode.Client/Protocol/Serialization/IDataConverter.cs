...
{
    /// <summary>
    /// Wire DSCode tag this converter handles. Used both as the
    /// registry decode key and as the byte written ahead of the
    /// payload on the encode side. Mirrors cppcache
    /// <c>Serializable::getDsCode()</c>.
    /// </summary>
    byte DsCode { get; }
    ...
    /// — the registry writes it before delegating in, so converters
    /// only emit body bytes.
    /// </summary>
    /// <param name="value">
    /// Boxed instance of <see cref="ManagedType"/>; concrete
    /// implementations unbox and forward to the generic
    /// <see cref="IDataConverter{T}.Write(BigEndianBinaryWriter, T)"/>.
    /// </param>
    void Write(BigEndianBinaryWriter writer, object value);

    /// <summary>
    /// Read one payload from <paramref name="reader"/>. The DSCode
    /// byte has already been consumed by the registry before this is
    /// called; converters only see body bytes.
    /// </summary>
    /// <returns>
    /// Boxed instance of <see cref="ManagedType"/>, or <c>null</c>
    /// for value types whose stored representation is "no value".
    /// </returns>
    object? Read(BigEndianBinaryReader reader);
}
