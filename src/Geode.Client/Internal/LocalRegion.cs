    ...
    RegionAttributes attributes) : RegionInternal(attributes)
{
    ...
    // base impls go here that consult m_entries first and delegate to
    // the derived class for server roundtrips.

    // TODO future phases — fields cppcache LocalRegion holds that we'll
    // grow into:
    ...
    //   m_attachedPool                    — Phase 1.x wiring (Step B+)
}
