...
/// </summary>
public class CachingProxyPutAsyncTests(IGeodeCacheFactory factory)
{
    ...
    {
        const string cacheName = nameof(CachingProxyPutAsyncTests);
        try
        {
    ...
    {
        const string cacheName = nameof(CachingProxyPutAsyncTests) + "_callback";
        try
        {
            var cache = await NewCacheAsync(cacheName);
            await cache.PoolManager.CreateFactory()
